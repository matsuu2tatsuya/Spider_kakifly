...
def test_read_csv(cleared_fs):
    ...
    cleared_fs.store["test/default.csv"] = MemoryFile(data=text)
    df2 = read_csv("memory://test/default.csv", parse_dates=["dt"])
...
def test_reasonable_error(monkeypatch, cleared_fs):
    ...
    with pytest.raises(ValueError) as e:
        read_csv("nosuchprotocol://test/default.csv")
    ...
    with pytest.raises(ImportError) as e:
        read_csv("couldexist://test/default.csv")
        assert err_mgs in str(e.value)


def test_to_csv(cleared_fs):
    df1.to_csv("memory://test/default.csv", index=True)
    df2 = read_csv("memory://test/default.csv", parse_dates=["dt"], index_col=0)
...
def test_to_parquet_new_file(monkeypatch, cleared_fs):
    ...
    df1.to_parquet(
        "memory://test/default.csv", index=True, engine="fastparquet", compression=None
    )
...
def test_not_present_exception():
    with pytest.raises(ImportError) as e:
        read_csv("memory://test/default.csv")
