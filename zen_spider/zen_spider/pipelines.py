...
class MySQLPipeline:
    ...
    def open_spider(self, spider):
        ...
        self.c.execute("""
            ...
            PRIMARY KEY (`buy_transaction_URL`)
            )
        """)
    ...
    def process_item(self, item, spider):
        ...
        Itemをitemsテーブルに挿入する。
        """
        self.c.execute('INSERT INTO `items`(`ID`,`name`,`price_ETH`, `price_SPL`,`price_JPY`,`buy_transaction_URL`,`sell_transaction_URL_ETH`,`buy_transaction_URL_JPY`,`image_URL`) '
                       'VALUES (%(ID)s,%(name)s,%(price_ETH)s,%(price_SPL)s,%(price_JPY)s,%(buy_transaction_URL)s,%(sell_transaction_URL_ETH)s,%(buy_transaction_URL_JPY)s,%(image_URL)s)', dict(item))
